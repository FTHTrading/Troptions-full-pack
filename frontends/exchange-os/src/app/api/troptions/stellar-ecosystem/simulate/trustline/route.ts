...
import { simulateStellarTrustline } from "@/lib/troptions/xrpl-stellar/xrplStellarControlHubBridge";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const result = simulateStellarTrustline({
      assetId: String(body.assetId ?? ""),
      holderKycStatus: String(body.holderKycStatus ?? "unknown") as "verified" | "pending" | "unknown" | "failed",
      issuerStatus: String(body.issuerStatus ?? "not_configured") as "live" | "testnet" | "draft" | "not_configured",
    });
