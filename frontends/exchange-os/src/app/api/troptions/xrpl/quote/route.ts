...
export async function POST(request: Request) {
  ...
  try {
    ...
    const quote = simulateConversionRoute(payload.sourceAsset ?? "USD.rIssuer", payload.targetAsset ?? "USDC", Number(payload.amount ?? 1000));
    const responseBody = { ok: true, quote, simulationOnly: true };

    auditPortalAction("xrpl_quote_simulated", guarded.auth.actorId, guarded.auth.actorRole, routeKey, {
