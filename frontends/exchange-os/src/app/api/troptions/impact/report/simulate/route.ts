...
import { simulateImpactReport } from "@/lib/troptions/impactReportingEngine";
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const body = (await request.json()) as { period?: string; program?: string };
    const result = simulateImpactReport({
      period: body.period ?? "current-quarter",
      program: body.program ?? "public-benefit",
    });

    trackControlPlaneEvent("impact_report_simulated", "info", {
