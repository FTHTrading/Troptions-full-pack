...
export const metadata = {
  title: "USDP Rail Evaluation | TROPTIONS Stablecoins",
  description: "USDP (Paxos Dollar) institutional evaluation for TROPTIONS — regulated Ethereum-native dollar stablecoin, treasury simulation route.",
};

const record = STABLECOIN_ISSUER_REGISTRY.find((r) => r.symbol === "USDP")!;

export default function TroptionsStablecoinsUsdpPage() {
  return (
    <main className="te-page">
      <div className="te-wrap" style={{ gap: "1.5rem" }}>
        ...
        <div style={{ background: "rgba(127,29,29,0.18)", border: "1px solid rgba(127,29,29,0.5)", borderRadius: "0.65rem", padding: "0.65rem 0.9rem", display: "flex", gap: "0.5rem", alignItems: "flex-start" }}>
          ...
          <p style={{ margin: 0, fontSize: "0.78rem", color: "#fca5a5", lineHeight: 1.5 }}><strong>Simulation Only.</strong> USDP is modeled as a regulated route for simulation and readiness workflows. No live execution without approval.</p>
        </div>

        <div className="te-panel" style={{ padding: "1.75rem" }}>
          <div style={{ display: "flex", alignItems: "flex-start", gap: "1rem", flexWrap: "wrap" }}>
            <Image src="/assets/troptions/logos/usdp-iou-logo.svg" alt="USDP logo" width={72} height={72} style={{ borderRadius: "50%", border: "1px solid rgba(0,184,160,0.4)", flexShrink: 0 }} />
            <div style={{ flex: 1 }}>
              <p className="te-kicker">Paxos Trust Company · Regulated Dollar</p>
              <h1 className="te-heading">USDP Rail Evaluation</h1>
              <p className="te-subheading" style={{ marginTop: "0.4rem", maxWidth: "620px" }}>Paxos Dollar (USDP) is a regulated, 1:1 USD-backed stablecoin issued by Paxos on Ethereum. Evaluated for TROPTIONS treasury simulation and regulated dollar route readiness.</p>
              <div style={{ display: "flex", flexWrap: "wrap", gap: "0.45rem", marginTop: "0.9rem" }}>
                ...
                <span style={{ background: "#001A1A", color: "#22d3ee", border: "1px solid #155e75", borderRadius: "0.3rem", padding: "0.15rem 0.5rem", fontSize: "0.68rem", fontWeight: 700 }}>PAXOS-REGULATED</span>
              </div>
            </div>
          </div>
        </div>
