...
export default function InvestorReadinessWorkflowPage() {
  return (
    <main className="min-h-screen bg-[#0A0A0A] text-white">
      <DisclaimerBanner variant="institutional" />
      <div className="max-w-6xl mx-auto px-6 py-16 space-y-8">
        <section>
          <p className="text-[#C9A84C] text-xs font-mono uppercase tracking-widest mb-2">Workflow - Investor Readiness</p>
          <h1 className="text-3xl font-bold">Troptions Investor Readiness Workflow</h1>
          <p className="text-slate-400 mt-2">KYC, sanctions, accreditation, documentation, and wallet allowlist gating.</p>
        </section>
            ...
            <div key={item.subjectId} className="bg-slate-900 border border-slate-800 rounded-xl p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <h2 className="text-white font-semibold">{item.investorName}</h2>
                  <p className="text-slate-400 text-xs">{item.investorType}</p>
                </div>
